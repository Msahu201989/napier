...
def lambda_handler(event, context):
    ...
    for item in alb_list:
        ...
        storedIPs = list(ip_dict[0].values())[1:]

        print("############# Load Balancer - " + alb_entry + " #############")
        print (storedIPs)

        for index, storedIP in enumerate(storedIPs):
            if storedIP in addr1[2]:
                storedIPs[index] = f"{storedIP} - exists"

        print (storedIPs)

        newIPIndexes = [storedIP for storedIP in storedIPs if "exists" not in storedIP]
        ...
        keysForUpdates = [key for index in newIPIndexes for key, value in ip_dict[0].items() if value == index]

        print ("### Keys where update needs to be made ###")
        print (keysForUpdates)

        ipforUpdates = [item for item in addr1[2] if item not in storedIPs]

        print ("###IPs to be replaced###")

        for index, ipforUpdate in enumerate(ipforUpdates):
            response = table.update_item(
                Key={
                    "alb_id": alb_entry
                },
                UpdateExpression="set "+keysForUpdates[index]+ "=:newIP",
                ExpressionAttributeValues={":newIP": ipforUpdate}
            )
            send_html_email(alb_entry, ip_dict[0][keysForUpdates[index]], ipforUpdate)
